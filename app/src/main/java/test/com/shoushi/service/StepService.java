...
public class StepService extends Service  implements SensorEventListener {
    ...
    //3秒进行一次存储
    private static int saveDuration = 600000;
    //传感器
    ...
    private class MessengerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                    case StringUtils.MSG_FROM_CLIENT:
                    try {
                        //这里负责将当前的步数发送出去，可以在界面或者其他地方获取，我这里是在MainActivity中获取来更新界面
                        Messenger messenger = msg.replyTo;
                        Message replyMsg = Message.obtain(null, StringUtils.MSG_FROM_SERVER);
...
//                        saveDuration = 3000;
    ...
    private void saveStepData() {
        ...
        builder.setContentIntent(PendingIntent.getActivity(this, 0, nfIntent, 0)) // 设置PendingIntent
                .setLargeIcon(BitmapFactory.decodeResource(this.getResources(), R.mipmap.launcher_ap)) // 设置下拉列表中的图标(大图标)
                .setContentTitle("今日步数"+CURRENT_STEP+"步") // 设置下拉列表里的标题
                .setSmallIcon(R.mipmap.launcher_ap) // 设置状态栏内的小图标
                .setContentText("加油，要记得勤加运动"); // 设置上下文内容　
        // 获取构建好的Notification
        Notification stepNotification = builder.build();
        //调用更新
